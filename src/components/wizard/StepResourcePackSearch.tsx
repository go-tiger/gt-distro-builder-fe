...
export default function StepResourcePackSearch({ mcVersion, onBack, onNext }: Props) {
  ...
  const buildParams = useCallback(
    (q: string, off: number) => {
      const facets = JSON.stringify([[`versions:${mcVersion}`], ['project_type:resourcepack']]);
  ...
  return (
    <div>
      ...
      <p className='text-sm text-[#475569] font-mono mb-6'>MC {mcVersion} 호환 리소스팩을 추가하세요</p>
                  ...
                  <div key={pack.project_id} className='flex items-start gap-3'>
                    ...
                    <div className='flex-1 min-w-0'>
                      ...
                      <div className='flex items-center gap-2'>
                        <TrackedToggle value={pack.tracked} onChange={v => updateValidate(pack.project_id!, v)} />
            ...
            <div className='mt-4 grid gap-2'>
              {manualSelected.map((pack, i) => (
                <div key={i} className='bg-[#0d1424] border border-[#1e2d45] rounded-xl p-3 flex items-center gap-3'>
                  ...
                  <TrackedToggle value={pack.tracked} onChange={v => updateManualValidate(pack.url, v)} />
                  <button
