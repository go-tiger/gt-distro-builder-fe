...
import type { SelectedResourcePack } from '@/types/wizard';
...
interface Props {
  mcVersion: string;
  onBack: () => void;
  onNext: (packs: SelectedResourcePack[]) => void;
}
...
export default function StepResourcePackSearch({ mcVersion, onBack, onNext }: Props) {
  ...
  // 선택된 항목
  const [selectedPacks, setSelectedPacks] = useState<SelectedResourcePack[]>([]);
  ...
  function updateValidate(project_id: string, tracked: boolean) {
  ...
  function addManual() {
    ...
    setSelectedPacks(prev => [
      ...prev,
      { type: 'manual', url, tracked: manualTracked },
    ]);
    setManualUrl('');
  ...
  function updateManualValidate(url: string | undefined, tracked: boolean) {
    setSelectedPacks(prev => prev.map(p =>
      p.type === 'manual' && p.url === url ? { ...p, tracked } : p
    ));
  }
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-[#e2e8f0] mb-2">리소스팩</h2>
      <p className="text-sm text-[#475569] font-mono mb-6">
        MC {mcVersion} 호환 리소스팩을 추가하세요
      </p>
        ...
        <>
          ...
          {!loading && (
            <div ref={scrollRef} className="grid gap-2 mb-6 max-h-96 overflow-y-auto pr-1">
              {results.map(pack => {
                ...
                return (
                  <div key={pack.project_id} className={`rounded-xl border transition-colors ${
                    selected
          ...
          {modrinthSelected.length > 0 && (
            <div className="bg-[#0d1424] border border-[#1e2d45] rounded-xl p-4 mb-6">
              <p className="font-mono text-xs text-[#475569] mb-3">선택된 리소스팩 ({modrinthSelected.length})</p>
                  ...
                  <div key={pack.project_id} className="flex items-start gap-3">
                    <div className="w-7 h-7 rounded-md overflow-hidden flex-shrink-0 bg-[#080c14] border border-[#1e2d45] mt-0.5">
                      {pack.icon_url ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-[#475569] text-[10px] font-mono">?</div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        <TrackedToggle
                          value={pack.tracked}
                          onChange={v => updateValidate(pack.project_id!, v)}
                        />
            ...
            <div className="mt-4 grid gap-2">
              {manualSelected.map((pack, i) => (
                <div key={i} className="bg-[#0d1424] border border-[#1e2d45] rounded-xl p-3 flex items-center gap-3">
                  ...
                  <TrackedToggle
                    value={pack.tracked}
                    onChange={v => updateManualValidate(pack.url, v)}
                  />
                  <button
