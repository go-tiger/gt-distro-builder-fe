...
function buildDistributionJson(
  ...
  const resourcePackModules: ModuleEntry[] = resourcePacks.map((pack, i) => {
    const id =
      pack.type === 'modrinth' && pack.project_id ? `${pack.project_id}:${pack.version_id}` : `resourcepack-${i}`;
    return {
      id,
      name: pack.title ?? id,
      type: 'File',
  ...
  const shaderPackModules: ModuleEntry[] = shaderPacks.map((pack, i) => {
    const id =
      pack.type === 'modrinth' && pack.project_id ? `${pack.project_id}:${pack.version_id}` : `shaderpack-${i}`;
    return {
      id,
      name: pack.title ?? id,
      type: 'File',
    ...
    servers: [
      {
        ...
        mainServer: true,
        autoconnect: false,
        modules: [loaderModule, ...modModules, ...resourcePackModules, ...shaderPackModules, ...extraFileModules],
      },
    ],
  };
}
...
export default function StepJsonPreview({
  ...
  return (
    <div>
      ...
      <p className='text-sm text-[#475569] font-mono mb-6'>distribution.json 을 확인하고 다운로드하세요</p>
      ...
      {!loadingBackend && backendModules && (
        <div className='bg-[#00d4aa]/5 border border-[#00d4aa]/20 rounded-xl px-4 py-2.5 mb-4'>
          <p className='font-mono text-xs text-[#00d4aa]'>백엔드에서 MD5 해시 {backendModules.length}개 적용됨</p>
        </div>
      )}
      ...
      {loader === 'fabric' && !loadingSubModules && loaderSubModules.length > 0 && (
        <div className='bg-[#0ea5e9]/5 border border-[#0ea5e9]/20 rounded-xl px-4 py-2.5 mb-4'>
          <p className='font-mono text-xs text-[#0ea5e9]'>Fabric 서브모듈 {loaderSubModules.length}개 자동 포함됨</p>
        </div>
      )}
      ...
      <div className='bg-[#0d1424] border border-[#1e2d45] rounded-xl overflow-hidden mb-6'>
        ...
        <pre className='font-mono text-xs text-[#94a3b8] p-4 overflow-auto max-h-96 leading-5'>{jsonStr}</pre>
      </div>
